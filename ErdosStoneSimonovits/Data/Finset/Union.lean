...
variable {α β : Type*} {s : Finset α}

section DisjiUnion

variable {s : Finset α} {t : Finset β} {f : α → β}

lemma pairwiseDisjoint_filter {f : α → Finset β} (h : (↑s : Set α).PairwiseDisjoint f)
...
theorem filter_disjiUnion (s : Finset α) (f : α → Finset β) (h) (p : β → Prop) [DecidablePred p] :
  ...
  ext b; simp [mem_filter, mem_disjiUnion, ← exists_and_right, and_assoc]
